...
#ifndef IMGUINOTIFY_HPP
#define IMGUINOTIFY_HPP
...
#define NOTIFY_USE_SEPARATOR \
    false  // If true, a separator will be rendered between the title and the content
   ...
   public:
    ...
    inline ImGuiToastPhase getPhase() {
        const int64_t elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(getElapsedTime()).count();
...
inline void RenderNotifications() {
    ...
    for (size_t i = 0; i < notifications.size(); ++i) {
        ...
        if (i > NOTIFY_RENDER_LIMIT) { continue; }
#endif
        ...
        const float opacity =
            currentToast->getFadePercent();  // Get opacity based of the current phase
        ...
        // Here we render the toast content
        {
            ...
            if (NOTIFY_USE_DISMISS_BUTTON) {
                ...
                if (CalcTextSize(content).x > GetContentRegionAvail().x) { scale = 0.8f; }
            ...
            if (currentToast->getOnButtonPress() != nullptr) {
                ...
                if (Button(currentToast->getButtonLabel())) { currentToast->getOnButtonPress()(); }
            }
